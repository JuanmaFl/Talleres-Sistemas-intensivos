...
echo "=== Ejecutando WordCount con MapReduce ==="
START=$(date +%s)
exec_namenode "hadoop jar ${EXAMPLES_JAR} wordcount ${HDFS_INPUT_DIR} ${HDFS_OUTPUT_DIR_MR}"
MR_TIME=$(($(date +%s) - START))

echo
echo "Tiempo MapReduce: ${MR_TIME}s"
echo
echo "Primeras lineas del resultado:"
exec_namenode "hdfs dfs -cat ${HDFS_OUTPUT_DIR_MR}/part-r-00000 | head -n 30"
